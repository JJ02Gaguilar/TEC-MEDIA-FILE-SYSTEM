...
# Leer configuración
def read_config():
    tree = ET.parse('config1.xml')  # Asegúrate de que este sea el correcto para disk_node1
    root = tree.getroot()
...
def store_block():
    ...
    content = data['data']
    with open(f"{storage_path}/{block_id}.blk", "w", encoding='utf-8') as f:
        f.write(content)
